...
unsigned int nStakeMaxAge = 365 * 24 * 60 * 60;
...
const string strMessageMagic = "Pearl Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	{ 
                nSubsidy = 64000000 * COIN; return nSubsidy;
	}
    else if(pindexBest->nHeight < 50)
    {
        nSubsidy = 0 * COIN;
    }
    else if(pindexBest->nHeight > 51)
    {
        nSubsidy = 100 * COIN;
    }
     else if(pindexBest->nHeight > 100051)
    {
            nSubsidy = 50 * COIN;
    }
     else if(pindexBest->nHeight > 300051)
    {
             nSubsidy = 25 * COIN;
    }
     else if(pindexBest->nHeight > 700051)
    {
             nSubsidy = 10 * COIN;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    int64_t nSubsidy = nCoinAge * nRewardCoinYear * 33 / (365 * 33);
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Pearl", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Pearl was released 1473811724";
        CTransaction txNew;
        txNew.nTime = 1473811724;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
		...
		if(fTestNet)
        {
            block.nNonce = 330305 ;
                ...
                uint256 nhashMerkleRoot = uint256("a63e445a47514894c5378df5341be7f11889bb40300aa096bf0223241205111f");
...
unsigned char pchMessageStart[4] = { 0x11, 0xb3, 0xa4, 0xd4 };
